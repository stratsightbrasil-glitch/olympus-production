...
export function RightPanel({ indicadores, weakSignals, signalStats, onRefreshIndicators, onRefreshSignals }: RightPanelProps) {
  const visible = indicadores.length > 0 || weakSignals.length > 0;
    ...
    <div style={{
      ...
      {indicadores.length > 0 && (
        <div style={{ padding: 16, borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#004D40', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
              📡 KRATOS · {total} indicador{total !== 1 ? 'es' : ''}
            </div>
            <button
              onClick={onRefreshIndicators}
              style={{ fontSize: 10, color: 'var(--text-ter)', background: 'none', border: 'none', cursor: 'pointer' }}
            >↻</button>
          </div>
      ...
      {weakSignals.length > 0 && (
        <div style={{ padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#4527A0', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
